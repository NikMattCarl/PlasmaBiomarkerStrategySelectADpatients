...
#Comparison of test AUC to p-tau217 alone
pvals=NULL
for (i in results_AB$biomarker) {
  roc_compare=roc.test(biomarkers_roc_test[["PL_ptau217"]],
           ...
           methods="delong",
           paired=TRUE)  
  pvals=c(pvals,roc_compare$p.value)
}
results_AB$pval_comp_ptau217_test=pvals
results_AB=results_AB[order(results_AB$AUC_cv10_train, decreasing = TRUE),]

#now compare with just P-tau217, without covariates
m_basic <- train(AB_status~PL_ptau217,
                 data = temp_train[,c("AB_status","PL_ptau217","age","gender","apoe4")], 
...
results_AB$pval_comp_ptau217_basic_test=pvals
results_AB=results_AB[order(results_AB$AUC_cv10_train, decreasing = TRUE),]
...
boot_ci(opt_cut_youden, sensitivity)
boot_ci(opt_cut_youden, specificity)
...
probs_try=seq(from=0.85,to=0.95, by=0.01)
for (k in 1:length(probs_try)) {
  ...
  train_set$Predicted_AB_class=ifelse(train_set$PL_ptau217_pgml_Lilly_2022_log10>=hi_cutpoint,"AB_Pos",
                                      ifelse(train_set$PL_ptau217_pgml_Lilly_2022_log10<=low_cutpoint,"AB_Neg",
  ...
  range_gz_train=prop.table(table(train_set$PL_ptau217_pgml_Lilly_2022_log10>low_cutpoint & train_set$PL_ptau217_pgml_Lilly_2022_log10<hi_cutpoint))["TRUE"]
  
  #bootstrap sens and spec
  #bootstrap for CI
  boot_results=data.frame(boot=numeric(),
                          sens_gz=numeric(),
                          spec_gz=numeric(),
                          range_gz=numeric())
  for (i in 1:1000) {
    boot_id=data.frame(sid=sample(train_set$sid, replace=TRUE), boot_sid=seq(1,nrow(train_set),1))#
    
    boot_id=left_join(boot_id, train_set, by="sid")
    
    boot_id$Predicted_AB_class=ifelse(boot_id$PL_ptau217>=hi_cutpoint,"AB_Pos",
                                      ifelse(boot_id$PL_ptau217<=low_cutpoint,"AB_Neg",
                                             as.character(boot_id$AB_status)))    
    
    sens_boot=table(boot_id$Predicted_AB_class,boot_id$AB_status)["AB_Pos","AB_Pos"]/(sum(table(boot_id$Predicted_AB_class,boot_id$AB_status)[,"AB_Pos"]))
    spec_boot=table(boot_id$Predicted_AB_class,boot_id$AB_status)["AB_Neg","AB_Neg"]/(sum(table(boot_id$Predicted_AB_class,boot_id$AB_status)[,"AB_Neg"]))
    
    range_gz_boot=prop.table(table(boot_id$PL_ptau217_pgml_Lilly_2022_log10>low_cutpoint & boot_id$PL_ptau217_pgml_Lilly_2022_log10<hi_cutpoint))["TRUE"]
    
    sub=data.frame(boot=i,
                   sens_gz=sens_boot,
                   spec_gz=spec_boot,
                   range_gz=as.numeric(range_gz_boot))
    boot_results=rbind(boot_results,sub)
  }
  boot_sens_ci=quantile(boot_results$sens, probs=c(0.025,0.975), na.rm=TRUE)
  boot_spec_ci=quantile(boot_results$spec, probs=c(0.025,0.975), na.rm=TRUE)
  boot_gz=quantile(boot_results$range_gz, probs=c(0.025,0.975), na.rm=TRUE)
  ...
  #bootstrap sens and spec, test set
  boot_results_test=data.frame(boot=numeric(),
                               sens_gz=numeric(),
                               spec_gz=numeric(),
                               range_gz=numeric())
  for (i in 1:1000) {
    ...
    sens_boot=table(boot_id$Predicted_AB_class,boot_id$AB_status)["AB_Pos","AB_Pos"]/(sum(table(boot_id$Predicted_AB_class,boot_id$AB_status)[,"AB_Pos"]))
    spec_boot=table(boot_id$Predicted_AB_class,boot_id$AB_status)["AB_Neg","AB_Neg"]/(sum(table(boot_id$Predicted_AB_class,boot_id$AB_status)[,"AB_Neg"]))
    range_gz_boot=prop.table(table(boot_id$PL_ptau217>low_cutpoint & boot_id$PL_ptau217<hi_cutpoint))["TRUE"]
    sub=data.frame(boot=i,
                   sens_gz=sens_boot,
                   spec_gz=spec_boot,
                   range_gz=as.numeric(range_gz_boot))
  ...
  boot_sens_ci_test=quantile(boot_results_test$sens, probs=c(0.025,0.975), na.rm=TRUE)
  boot_spec_ci_test=quantile(boot_results_test$spec, probs=c(0.025,0.975), na.rm=TRUE)
  boot_gz_test=quantile(boot_results_test$range_gz, probs=c(0.025,0.975), na.rm=TRUE)
...
#use optimal cutpoints for gray zone to predict AB class
...
#bootstrap sens and spec in training set
boot_results=data.frame(boot=numeric(),
                        sens_gz=numeric(),
                        spec_gz=numeric(),
                        range_gz=numeric())
for (i in 1:1000) {
  ...
  sub=data.frame(boot=i,
                 sens_gz=sens_boot,
                 spec_gz=spec_boot,
                 range_gz=range_gz_boot)
  boot_results=rbind(boot_results,sub)
}
boot_sens_ci=quantile(boot_results$sens, probs=c(0.025,0.975))
boot_spec_ci=quantile(boot_results$spec, probs=c(0.025,0.975))
boot_gz=quantile(boot_results$range_gz, probs=c(0.025,0.975))
